...
class LoadStreams:  # 웹캠 및 스트리밍 전용 로더
    def __init__(self, sources='0', img_size=640, stride=32):
        ...
        for i, s in enumerate(sources):
            ...
            _, self.imgs[i] = cap.read()  # 첫 프레임 읽기
            self.cap = cap # 기존 코드 호환용
    ...
    def __next__(self):
        ...
        # LoadImages와 동일한 전처리 루틴 (1280x720 -> letterbox)
        img0 = cv2.resize(img0, (1280, 720), interpolation=cv2.INTER_LINEAR)
        ...
        # 4번째 인자로 패딩 정보를 넘겨주어 나중에 복원할 수 있게 함
        return 'webcam.jpg', img, img0, (ratio, (dw, dh))
