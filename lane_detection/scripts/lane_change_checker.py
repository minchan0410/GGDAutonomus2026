...
class SideState:
    ...
    def update(self, x, dt):
        # Returns x, ema, ema_dx, ema_ddx, ema_dddx
        ...
        if self.last_ema is not None:
            ...
            if self.last_ema_dx is not None:
                ...
                if self.last_ema_ddx is not None:
                    ema_dddx = (ema_ddx - self.last_ema_ddx) / dt
        self.last_ema = self.ema
        self.last_ema_dx = ema_dx
        self.last_ema_ddx = ema_ddx

        self.last_valid = True
        return x, self.ema, ema_dx, ema_ddx, ema_dddx
...
class LaneChangeChecker:
    def __init__(self):
        ...
        self.left_thresh = {
            ...
            'ema_dddx': float(_get_param(ns, 'left_ema_dddx_thresh', DEFAULT_LEFT_EMA_DDDX_THRESH)),
        }
        self.right_thresh = {
            ...
            'ema_dddx': float(_get_param(ns, 'right_ema_dddx_thresh', DEFAULT_RIGHT_EMA_DDDX_THRESH)),
        }
        ...
        self.use_abs = {
            ...
            'ema_dddx': bool(_get_param(ns, 'use_abs_ema_dddx', DEFAULT_USE_ABS_EMA_DDDX)),
        }

        self.left_state = SideState(self.ema_window)
        self.right_state = SideState(self.ema_window)
        ...
        self.pub_left_ema_dddx = rospy.Publisher('lane_change/left/ema_dddx', Float32, queue_size=10)
        ...
        self.pub_right_ema_dddx = rospy.Publisher('lane_change/right/ema_dddx', Float32, queue_size=10)
    ...
    def _check_thresholds(self, values, thresh_map):
        # values: dict with keys x, ema, ema_dx, ema_ddx, ema_dddx
    ...
    def _publish_metrics(self, pubs, values, invalid_value):
        # pubs: list [x, ema, ema_dx, ema_ddx, ema_dddx]
        out = []
        for v in values:
    ...
    def timer_callback(self, event):
        ...
        self._publish_metrics(
            [
                self.pub_left_x, self.pub_left_ema, self.pub_left_ema_dx,
                self.pub_left_ema_ddx, self.pub_left_ema_dddx
            ],
        ...
        self._publish_metrics(
            [
                self.pub_right_x, self.pub_right_ema, self.pub_right_ema_dx,
                self.pub_right_ema_ddx, self.pub_right_ema_dddx
            ],
        ...
        if left_metrics:
            left_values = {
                'x': left_metrics[0],
                'ema': left_metrics[1],
                'ema_dx': left_metrics[2],
                'ema_ddx': left_metrics[3],
                'ema_dddx': left_metrics[4],
            }
        if right_metrics:
            right_values = {
                'x': right_metrics[0],
                'ema': right_metrics[1],
                'ema_dx': right_metrics[2],
                'ema_ddx': right_metrics[3],
                'ema_dddx': right_metrics[4],
            }
